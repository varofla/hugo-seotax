...
- URL 상태는 `query`, `category1`, `category2`, `tags`, `tagsOp`, `page`, `pageSize`. 기존 URL의 `sort`는 무시하며 새 링크에는 포함하지 않는다. `params.search.sort` 설정도 사용하지 않는다. 태그 링크는 `/search/?tags=...`로 간다.
- Fuse 점수는 검색 모달과 전체 결과 페이지가 함께 사용한다. 키워드가 있으면 점수 오름차순 → 최신 날짜 → 숫자 ID 오름차순으로, 없으면 최신 날짜 → 숫자 ID 오름차순으로 결과를 고정한다. 정렬은 필터 적용 후 페이지네이션 전에 수행한다.
