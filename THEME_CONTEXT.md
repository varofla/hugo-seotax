...
- 모바일에도 같은 디자인 기조를 적용한다. 모바일 메뉴·목차 형태나 제스처의 구체적인 사양은 아직 결정하지 않았다.
...
- 모바일 헤더에 메뉴 아이콘, 검색 영역, 목차 아이콘이 있다(`header/mobile.html`).
- 왼쪽 메뉴는 checkbox 상태와 transform을 이용한 서랍 형태이며 overlay로 닫을 수 있다.
- 목차는 기본 숨김이며 `toc-toggle.js`가 `.overlay-mode`와 배경 overlay를 켠다. 닫기 버튼·배경 클릭·Escape·데스크톱으로 resize하는 경로가 있다.
- 메뉴와 목차의 상태를 각각 관리한다. 둘을 연속으로 여는 경우의 배경·포커스·스크롤 관계는 브라우저에서 확인해야 한다.
- 글/ About의 주요 전환 SCSS는 데스크톱 조건 안에 있다. 모바일에 동일 전환이 구현되었다고 볼 수 없다.
...
- 브라우저 렌더링·모바일 실기기·전환 애니메이션은 이번에 실행 검증하지 않았다. 빌드 성공이 인터랙션 정상 동작을 의미하지 않는다.
- 위 임시 경로는 분석 당시 결과물이다. `/tmp`가 정리되면 재생성해야 하며 복사된 테마는 이후 수정과 자동 동기화되지 않는다.

## 10. 사용자에게 확인할 시점

**결정 완료:** 범용 테마 지원보다 varofla 전용 관리에 초점을 두고, 한국어 단일 언어만 제공한다. `exampleSite`와 원본 배포 workflow는 제거했다.

아래는 문서 작성을 막지 않으며 실제 해당 작업을 시작할 때 확인한다. 지금 임의로 선택하지 않았다.

- 청소: 미사용 shortcode, 현재 공사 공지, SW/offline 유지 여부 등 실제 기능·운영 범위를 바꾸는 항목.
- 모바일: 읽는 동안 헤더의 노출 방식, 메뉴·목차의 표시 형태, 이미지 pinch/swipe 지원 범위. 기존 최소 UI 원칙을 전제로 구체안을 보여주고 결정한다.
